...
class ImageOverlayProcessor:
    def __init__(self):
        ...
        #self.overlay_image = cv2.imread(r'/home/soslab/Downloads/PSYCH_EDITED/1.png')
        #self.overlay_image = cv2.imread(r'/home/soslab/Downloads/Style_Transfer_Edited/1.png')
        ...
        self.buffer_size= 25
        self.binarized_images_buffer = deque(maxlen=self.buffer_size)

       # New attributes for mask blending
        self.current_mask = None
        self.next_mask = None
        self.mask_blend_factor = 0  # Start with the first mask fully in effect

        # Timer setup
        self.update_interval = 0.06  # Interval in seconds to update the overlay image
        rospy.Timer(rospy.Duration(self.update_interval), self.timer_callback)

        self.display_duration = 4  # Display each image for N seconds before starting to blend
        self.transition_duration = 6  # Transition to the next image over M seconds
        ...
        #self.image_directory = r'/home/soslab/Downloads/PSYCH_EDITED/'
        #self.image_directory = r'/home/soslab/Downloads/Style_Transfer_Edited/'

        # Initialize an empty list to store the images
        # image_list = []

        # Loop through the numbers, construct each filename, read the image, and append it to the list
        for i in range(1, 51):  # This will iterate through numbers 1 to 5
    ...
    def binarized_callback(self, data):
        ...
        """
        Updated callback for binarized images. Prepares masks for blending.
        """
        new_mask = self.bridge.imgmsg_to_cv2(data)
        # Convert new_mask to boolean or another appropriate format for blending
        new_mask = new_mask > 0

        # Update masks for blending
        if self.current_mask is None:
            self.current_mask = new_mask
        else:
            self.next_mask = new_mask
            self.mask_blend_factor = 0  # Reset blend factor for new transition
        #New
        try:
            self.mask_image = self.bridge.imgmsg_to_cv2(data)
    ...
    # def process_images_fast(self):
    #     """
    #     Processes images to apply the overlay.
    #     """
    ...
    def process_images_fast(self):
        """
        Processes images to apply the overlay, accumulating the effect of the last N binarized images.
        """
        if self.last_binarized_image is None:
            rospy.logwarn("No binarized image received yet. Skipping processing.")
            return
        ...
        # Update the buffer with the latest binarized image
        self.binarized_images_buffer.append(self.last_binarized_image > 0)

        # Initialize an accumulator mask to combine the effects of the last N masks
        accumulator_mask = np.zeros_like(self.last_binarized_image, dtype=bool)

        # Combine the masks in the buffer
        for mask in self.binarized_images_buffer:
            accumulator_mask = np.logical_or(accumulator_mask, mask)

        if self.overlay_image.shape[:2] != self.last_binarized_image.shape[:2]:
            rospy.logerr("Overlay image and mask image have different dimensions.")
            return

        # Apply the accumulated mask to overlay the image onto the background
        self.background_image[accumulator_mask] = self.overlay_image[accumulator_mask]
